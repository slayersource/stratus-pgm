 ...
 * {@link net.minecraft.server.CommandReplaceItem}. If we ever implement
 * applying kits to other types of inventories, this should be expanded to
 * include those slot names as well.
 */
public abstract class Slot {
	...
	public int getIndex() {
		if (isAuto())
			throw new UnsupportedOperationException("The auto-slot has no index");
		return index;
	}
	...
	/**
	 * @return true if this is the special "auto" slot. Putting a stack in this slot
	 *         will merge it with the inventory by calling {@link Inventory#addItem}
	 */
	public boolean isAuto() {
		return index < 0;
	}

	// If we support more inventory types, we would probably widen the type accepted
	// by this method
	...
	/**
	 * @return the item in this slot of the given holder's inventory, or null if the
	 *         slot is empty. This will never return a stack of
	 *         {@link Material#AIR}.
	 */
	 ...
	 *
	 * @return a stack of any items that were NOT placed in the inventory, or null
	 *         if the entire stack was placed. This can only be non-null when
	 *         placing in the auto-slot.
	 */
	...
	public static class Player extends Slot {
		...
		/**
		 * Convert a {@link PlayerInventory} slot index to a {@link Slot} object.
	 ...
	 * The "slot." at the beginning of the name is optional. Returns null if the
	 * name is invalid.
	 */
